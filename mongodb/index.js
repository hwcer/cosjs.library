...
class cosjs_mongodb{
    ...
    get(id){
        ...
        let option = {"multi":this.util.isMulti(id),"projection":this.util.fields(key)};
    ...
    count(query){
        ...
        this.collection((err,coll)=>{
            ...
            if(typeof coll.countDocuments === 'function'){
                coll.countDocuments(query,options,callback);
            }
            else{
...
function mongodb_connect(opts,callback) {
    ...
    if(opts instanceof MongoClient){
        ...
        MongoClient.connect(mongodbUrl,MongoOption,callback)
    }
    else {
        let encodeUrl = mongodb_url_encode(opts);
        let mongodbUrl = 'mongodb://' + encodeUrl;
        MongoClient.connect(mongodbUrl, MongoOption, callback)
    }
}
